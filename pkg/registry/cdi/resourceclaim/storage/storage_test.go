...
func validNewResourceClaim(name, ns string) *cdi.ResourceClaim {
	claim := &cdi.ResourceClaim{
		...
		Spec: core.ResourceClaimSpec{
			...
			Parameters:        runtime.RawExtension{Raw: []byte(`{"size":1000}`)},
			AllocationMode:    core.AllocationModeImmediate,
		},
		Status: cdi.ResourceClaimStatus{
			Phase: cdi.ResourceClaimAllocated,
			Scheduling: &cdi.SchedulingStatus{
				Scheduler: cdi.SchedulerSchedulingStatus{
					SelectedNode: "some-node",
				},
			},
			UsedOnNodes: []string{"worker-1"},
		},
	}
	return claim
}
...
func TestWatchCustomFieldSelectedNode(t *testing.T) {
	storage, _, server := newStorage(t)
	defer server.Terminate(t)
	defer storage.Store.DestroyFunc()
	test := genericregistrytest.New(t, storage.Store)
	test.TestWatch(
		...
		// matching fields
		[]fields.Set{
			{"status.scheduling.scheduler.selectedNode": "some-node"},
		},
		// not matching fields
		[]fields.Set{
			{"status.scheduling.scheduler.selectedNode": "some-other-node"},
		},
	)
}

func TestWatchCustomFieldUsedOnNodes(t *testing.T) {
	storage, _, server := newStorage(t)
	defer server.Terminate(t)
	defer storage.Store.DestroyFunc()
	test := genericregistrytest.New(t, storage.Store)
	test.TestWatch(
		...
		// matching fields
		[]fields.Set{
			{"status.usedOnNodes.worker-1": "1"},
		},
		// not matching fields
		[]fields.Set{
			{"status.usedOnNodes.worker-2": "1"},
		},
	)
}
...
func TestUpdateStatus(t *testing.T) {
	...
	claim.Status.Phase = cdi.ResourceClaimReallocate
	...
	claimOut := obj.(*cdi.ResourceClaim)
