...
import (
	...
	"k8s.io/kubernetes/pkg/apis/cdi/validation"
...
func (resourceclaimStrategy) PrepareForUpdate(ctx context.Context, obj, old runtime.Object) {
	newClaim := obj.(*cdi.ResourceClaim)
	oldClaim := old.(*cdi.ResourceClaim)
...
func GetAttrs(obj runtime.Object) (labels.Set, fields.Set, error) {
	claim, ok := obj.(*cdi.ResourceClaim)
	if !ok {
...
func toSelectableFields(claim *cdi.ResourceClaim) fields.Set {
	fields := generic.ObjectMetaFieldsSet(&claim.ObjectMeta, true)
	fields["status.scheduling.scheduler.selectedNode"] = claim.Status.Scheduling.Scheduler.SelectedNode
	for _, node := range claim.Status.UsedOnNodes {
		fields["status.usedOnNodes."+node] = "1"
	}
	return fields
}
