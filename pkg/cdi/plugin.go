...
const (
	// CDIPluginName is the name of the in-tree CDI Plugin
	CDIPluginName = "kubernetes.io/cdi"

	resourceTimeout = 2 * time.Minute
)

// cdiPlugins map keep track of all registered CDI plugins on the node and their
// corresponding sockets
var cdiPlugins = &PluginsStore{}
...
func (h *RegistrationHandler) RegisterPlugin(pluginName string, endpoint string, versions []string) error {
	...
	// Storing endpoint of newly registered CDI Plugin into the map, where plugin name will be the key
	// all other CDI components will be able to get the actual socket of CDI plugins by its name.
	cdiPlugins.Set(pluginName, Plugin{
		endpoint:                endpoint,
...
func highestSupportedVersion(versions []string) (*utilversion.Version, error) {
	if len(versions) == 0 {
		return nil, errors.New(log("CDI driver reporting empty array for supported versions"))
	}
...
func (h *RegistrationHandler) validateVersions(callerName, pluginName string, endpoint string, versions []string) (*utilversion.Version, error) {
	if len(versions) == 0 {
		return nil, errors.New(log("%s for CDI driver %q failed. Plugin returned an empty list for supported versions", callerName, pluginName))
	}

	// Validate version
	newDriverHighestVersion, err := highestSupportedVersion(versions)
	if err != nil {
		return nil, errors.New(log("%s for CDI driver %q failed. None of the versions specified %q are supported. err=%v", callerName, pluginName, versions, err))
	}

	existingDriver, driverExists := cdiPlugins.Get(pluginName)
	if driverExists {
		if !existingDriver.highestSupportedVersion.LessThan(newDriverHighestVersion) {
			return nil, errors.New(log("%s for CDI driver %q failed. Another driver with the same name is already registered with a higher supported version: %q", callerName, pluginName, existingDriver.highestSupportedVersion))
		}
	}
...
// of a new registration socket opened by CDI plugin.
func (h *RegistrationHandler) ValidatePlugin(pluginName string, endpoint string, versions []string) error {
	klog.Infof(log("Trying to validate a new CDI plugin with name: %s endpoint: %s versions: %s",
		pluginName, endpoint, strings.Join(versions, ",")))

	_, err := h.validateVersions("ValidatePlugin", pluginName, endpoint, versions)
	if err != nil {
		return fmt.Errorf("validation failed for CDI plugin %s at endpoint %s: %v", pluginName, endpoint, err)
	}
...
func log(msg string, parts ...interface{}) string {
	return fmt.Sprintf(fmt.Sprintf("%s: %s", CDIPluginName, msg), parts...)
}
