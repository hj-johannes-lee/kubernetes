...
type ResourceClaimStatusApplyConfiguration struct {
	Phase       *v1alpha1.ResourceClaimPhase          `json:"phase,omitempty"`
	...
	ReservedFor []v1.OwnerReferenceApplyConfiguration `json:"reservedFor,omitempty"`
	UsedOnNodes []string                              `json:"usedOnNodes,omitempty"`
}
...
func (b *ResourceClaimStatusApplyConfiguration) WithPhase(value v1alpha1.ResourceClaimPhase) *ResourceClaimStatusApplyConfiguration {
	b.Phase = &value
	return b
}
...
func (b *ResourceClaimStatusApplyConfiguration) WithScheduling(value *SchedulingStatusApplyConfiguration) *ResourceClaimStatusApplyConfiguration {
	b.Scheduling = value
	return b
}
...
func (b *ResourceClaimStatusApplyConfiguration) WithReservedFor(values ...*v1.OwnerReferenceApplyConfiguration) *ResourceClaimStatusApplyConfiguration {
	for i := range values {
		if values[i] == nil {
...
// WithUsedOnNodes adds the given value to the UsedOnNodes field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the UsedOnNodes field.
func (b *ResourceClaimStatusApplyConfiguration) WithUsedOnNodes(values ...string) *ResourceClaimStatusApplyConfiguration {
	for i := range values {
		b.UsedOnNodes = append(b.UsedOnNodes, values[i])
	}
	return b
}
